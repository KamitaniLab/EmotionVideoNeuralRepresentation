...
function[h,L,MX,MED,bw]=violin(Y,varargin)
 ...
 for i=i:size(Y,2)
     ...
     if plotci==1
         hold on
     plot([interp1(U(:,i),F(:,i)+i,MX(:,i)+ci1), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i)+ci1) ],[MX(:,i)+ci1 MX(:,i)+ci1],'r','LineWidth',0.5);
     plot([interp1(U(:,i),F(:,i)+i,MX(:,i)-ci1), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i)-ci1) ],[MX(:,i)-ci1 MX(:,i)-ci1],'r','LineWidth',0.5);
     end
     hold on
     if setX == 0
         if plotmean == 1
             p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i)) ],[MX(:,i) MX(:,i)],mc,'LineWidth',0.5);
         end
         if plotmedian == 1
             p(2)=plot([interp1(U(:,i),F(:,i)+i,MED(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MED(:,i)) ],[MED(:,i) MED(:,i)],medc,'LineWidth',0.5);
         end
     elseif setX == 1
         if plotmean == 1
             p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i))+x(i)-i, interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i))+x(i)-i],[MX(:,i) MX(:,i)],mc,'LineWidth',0.5);
         end
         if plotmedian == 1
             p(2)=plot([interp1(U(:,i),F(:,i)+i,MED(:,i))+x(i)-i, interp1(flipud(U(:,i)),flipud(i-F(:,i)),MED(:,i))+x(i)-i],[MED(:,i) MED(:,i)],medc,'LineWidth',0.5);
         end
     end
 end
 %-------------------------------------------------------------------------
